...
tours_controller = Blueprint("tours_controller", __name__, url_prefix = "/api/Tours")
toursService = ToursService()
accountService = AccountService()

@tours_controller.route("/GetToursForManager", methods=["GET"])
...
@tours_controller.route("/GetToursForCustomer", methods=["GET"])
...
@tours_controller.route("/DeleteTour", methods=["POST"])
def delete_tour():
    tourId = request.get_json()
    toursService.delete_tour(tourId)
    return 'Ok'

@tours_controller.route("/PayForTour", methods=["POST"])
def pay_for_tour():
    tourId = request.get_json()
